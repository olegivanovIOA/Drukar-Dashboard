...
def parse_production_from_alldata(rows):
    ...
    for row in rows[1:]:
        ...
        if not ym or ym < '2025-11': skipped_no_date += 1; continue
        ...
        if line_key in seen_lines: skipped_dedup += 1; continue
    ...
    print(f"  Production rows: counted={counted}, skipped_no_date={skipped_no_date}, skipped_dedup={skipped_dedup}, skipped_no_vid={skipped_no_vid}")
    # Квітень 2026 детально
    apr = monthly.get('2026-04', {})
    may = monthly.get('2026-05', {})
    print(f"  2026-04: PETG={apr.get('petg',0):.1f}, PLA={apr.get('pla',0):.1f}, packed_PETG={apr.get('petg_packed',0):.1f}")
    print(f"  2026-05: PETG={may.get('petg',0):.1f}, PLA={may.get('pla',0):.1f}")
