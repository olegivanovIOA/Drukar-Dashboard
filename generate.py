...
def _mc9sl_num(v):
    """Парсить число з gviz-CSV. КРИТИЧНО: великі суми gviz віддає у US-стилі з
    комою як роздільником ТИСЯЧ ("3,124,477"), а не як укр. десятковий роздільник —
    попередня версія сліпо міняла кожну кому на крапку і перетворювала це на
    "3.124.477" (дві крапки) → float() падав → тихо повертало 0.0 для КОЖНОЇ суми
    (тоннаж без ком типу "25"/"1.3" через це парсився нормально, тому баг був
    непомітний, поки не подивились саме на грошові стовпці)."""
    s = str(v).strip().replace('\xa0', '').replace(' ', '')
    if not s:
        return 0.0
    try:
        if s.count(',') > 1:
            # кілька ком поспіль = напевно роздільники тисяч (US-стиль "3,124,477")
            s = s.replace(',', '')
        elif ',' in s and '.' not in s:
            # одна кома, без крапки: ≤2 цифри після — десятковий роздільник
            # (укр. "1,3" т); 3 цифри — роздільник тисяч (одна група, напр. "3,124")
            head, tail = s.rsplit(',', 1)
            s = (head + '.' + tail) if len(tail) <= 2 else (head + tail)
        elif ',' in s and '.' in s:
            # і кома, і крапка одночасно — стандартний US-формат: кома=тисячі
            s = s.replace(',', '')
        return float(s)
